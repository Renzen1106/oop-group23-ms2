...
public class PayslipSplitDialog extends JDialog {
    ...
    public PayslipSplitDialog(Frame owner, Employee e) {
        ...
        for (JComponent f : new JComponent[]{idF, lnF, fnF, bdF, phF, sssF, philF, tinF, pagF, stF, posF, supF}) {
            if (f instanceof JTextField) {
                ((JTextField) f).setEditable(false);
            }
        }
        ...
        for (JTextField f : new JTextField[]{periodF, earnedF, riceF, phAllF, clAllF, grossF, sssDedF, philDedF, pagDedF, taxDedF, totDedF, netF}) {
            f.setEditable(false);
    ...
    private void doCompute() {
        ...
        BigDecimal earned = payrollService.computeMonthlyPay(emp, ym);
        ...
        BigDecimal sssDed = payrollService.computeSssDeduction(gross);
        BigDecimal philDed = payrollService.computePhilHealthDeduction(gross);
        BigDecimal pagDed = payrollService.computePagIbigDeduction(gross);
        sssDedF.setText(sssDed.toPlainString());
        philDedF.setText(philDed.toPlainString());
        pagDedF.setText(pagDed.toPlainString());

        BigDecimal tax = payrollService.computeWithholdingTax(earned, sssDed, philDed, pagDed);
