...
public class ValidationUtil {
    ...
    public static boolean isValidSSS(String sss) {
        ...
        return sss.trim().matches("\\d{2}-\\d{7}-\\d"); // Example: 44-4506057-3
    }

    public static boolean isValidPhilHealth(String philhealth) {
        if (philhealth == null) return false;
        return philhealth.trim().matches("\\d{12}"); // Example: 820126853951
    }

    public static boolean isValidTIN(String tin) {
        if (tin == null) return false;
        return tin.trim().matches("\\d{3}-\\d{3}-\\d{3}-\\d{3}"); // Example: 442-605-657-000
    }

    public static boolean isValidPagibig(String pagibig) {
        if (pagibig == null) return false;
        return pagibig.trim().matches("\\d{12}"); // Example: 691295330870
    }
    ...
    public static boolean isValidSupervisor(String supervisor) {
        if (supervisor == null || supervisor.isBlank()) return false;
